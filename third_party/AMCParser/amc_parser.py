...
class Joint:
  ...
  def set_motion(self, motion): 
    '''
    actually, this function performs inverse kinematics
    '''
    # print("\'{}\':{},".format(self.name, str([x.name for x in self.children])))
    # print([x.name for x in self.children]  )
    # print()
    if self.name == 'root':
      self.coordinate = np.reshape(np.array(motion['root'][:3]), [3, 1])*self.scale
      rotation = np.deg2rad(motion['root'][3:])
      self.matrix_local = euler2mat(*rotation)
      self.matrix = self.C.dot(self.matrix_local).dot(self.Cinv)
      # import pdb
      # pdb.set_trace()
      dt = self.direction.T[0]
      # print("[{},{},{}]".format(dt[0], dt[1], dt[2]))
    else:
      idx = 0
      rotation = np.zeros(3)
      ...
      self.matrix_local = euler2mat(*rotation)
      self.matrix = self.parent.matrix.dot(self.C).dot(self.matrix_local).dot(self.Cinv)
...
def test_all():
  ...
  for lv1 in lv1s:
    ...
    amc_path = asf_path.replace('{}.asf'.format(lv1), '{}_01.amc'.format(lv1))
